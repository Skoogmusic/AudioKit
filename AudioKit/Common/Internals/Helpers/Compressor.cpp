...
Compressor::Compressor(float fThreshold, float fRatio, float fAttack, float fRelease, int iSampleRate)
:
envDetector((double)iSampleRate),
compGain(0.0f)
{
...
float Compressor::calcCompressorGain(float fDetectorValue, float fTheThreshold, float fTheRatio, float fKneeWidth, bool bLimit)
{
    // slope variable
...
float Compressor::getCompGain()
{
    return this->compGain;
}
